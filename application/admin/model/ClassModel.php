...
class ClassModel extends Model
{
    ...
    public function getDetail($id){
        $data = $this->field(['id','name'])->where(['id'=>$id,'delete_time'=>0])->find()->getData();
        return $data;
    }
    /**
     * 添加班级
    ...
    public function doAdd($data){
        $result = $this->insert(['name' => $data['name']]);
        if ($result) {
    ...
    public function doEdit($data){
        ...
        $result = $this->where(['id'=>$data['id'],'delete_time'=>0])->update(['name' => $data['name']]);
        if ($result) {
